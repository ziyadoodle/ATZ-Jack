...
import chips from "./../assets/cards/PokerChips.png";
...
function Game() {
  ...
  var [chip, setChip] = useState(200);

  useEffect(() => {
    console.log(localStorage.getItem('chip'));
  }, [chip]);
  ...
  return (
    ...
    <p className="fixed top-8 md:top-10 right-3 xl:right-[20rem] 2xl:right-[40rem] flex items-center justify-center text-2xl w-28 h-12 rounded-md bg-primary text-bg">
      ${chip}
      <img className="w-10 h-10 ml-2" src={chips} alt="chips" />
    </p>
    {/* <p className="text-2xl text-secondary antialiased underline text-center mt-6">ATZ Jack</p> */}
    <Link className="text-3xl text-secondary underline mt-6 xl:mt-4 2xl:mt-8 text-center rounded-md" to={"/help"}>how to play?</Link>

      {/* dealer hand */}
      <div id="dealerHand" className="flex justify-center mt-10 xl:mt-8 2xl:mt-14 relative scale-[0.8] flex-wrap w-[300px]">
        <h1 className={`text-7xl text-primary absolute -top-8 -right-10 ${winner == 'you' ? 'line-through' : ''} `}>{!canPlay ? dealerSum : hiddenScore.pop()?.value}</h1>
        ...
        {dealerDeck.map((card, index) => {
          if (canPlay) {
            ...
            return <div key={index} className={`flex justify-center items-center text-4xl text-[#803535]  bg-cover w-24 h-36 -ml-6 ${hidden}`}>{index != 0 ? card.figure : ''}</div>
          } else {
            const sendToAbove = index > 2 ? `absolute left-${index > 3 ? 12 : 6} top-28` : '';
            const animateNewCard = index > 1 ? 'animate-getCard' : '';
            return <div key={index} className={`flex justify-center items-center text-4xl text-[#803535] bg-[url('/src/assets/cards/card.png')] bg-cover w-24 h-36 -ml-6 ${animateNewCard} ${sendToAbove}`}>{card.figure}</div>
          }
        })}
      </div>
      ...
      {/* your hand */}
      <div id="playerHand" className="flex justify-center mt-20 xl:mt-10 2xl:mt-20 relative scale-[0.7] flex-wrap w-[350px]">
        <h1 className={`text-7xl text-primary absolute -top-8 -right-10 ${winner == 'dealer' ? 'line-through' : ''}`}>{yourSum}</h1>

        {yourDeck.map((card, index) => {
          const sendToAbove = index > 2 ? `absolute sendToAbove -bottom-32` : '';
          return <div key={index} className={`flex justify-center items-center text-6xl text-[#803535] bg-[url('/src/assets/cards/card.png')] bg-cover w-32 h-48 -ml-6 animate-getCard -rotate-2 ${sendToAbove}`}>{card.figure}</div>
        })}

        {yourSum > 21 ? <h1 className="text-red-500 text-6xl absolute top-0 z-50 animate-getAlert">exceeded</h1> : null}
        {winner != '' ? <h1 className="text-primary text-6xl absolute -top-10  z-50 animate-getAlertWinner flex items-center">{winner} {winner == 'tie' ? '' : 'win'} {winner == 'you' ? '+10' : '-10'} {winner == 'you' ? <img className="w-8 h-8 mt-2" src={chips} alt="chips" /> : <img className="w-8 h-8 mt-2" src={chips} alt="chips" />}</h1> : null}
      </div>

      <div className="bg-[url('/src/assets/cards/yourShadow.png')] bg-inherit bg-no-repeat w-60 h-10 mt-8 xl:mt-5 2xl:mt-10 relative scale-[0.8] md:scale-100 -z-50"></div>
